...
experiment_name = "SPPS_Chemputer_Diazirine-NH-VGSA-OH"
...
########################################################################################################################
#                                                                                                                      #
#       NHS-DIAZIRIN COUPLING                                                                                          #
#                                                                                                                      #
#                                                                                                                      #
########################################################################################################################
c.logger.info("Coupling of the NHS_diazirine residue.")
c.camera.change_recording_speed(20)
...
#Prepare NHS_diazirine Solution
c.move("DCM", "NHS_diazirine", volume_diazirine_solution, speed=default_speed_slow)
c.move("DCM", "Waste_2", 10, speed=default_speed_fast) #clean the waste port to valve_2

total_volume_precleavage_solvent += 10 + volume_diazirine_solution

for i in range(3): 
	c.move("Waste_2", "Waste_2", 10, speed=default_speed_fast) #push air out of waste port of valve_2

#Mix diazirine solution by bubbling air though it
for i in range(3): 
	c.move("Waste_2", "NHS_diazirine", 10, speed=default_speed_slow)
	c.wait(60)

#Mix diazirine solution by pumping it up and down
for i in range(5):
	c.move("NHS_diazirine", "Pump_2", 5, speed=default_speed_slow)
	c.move("Pump_2", "NHS_diazirine", 5, speed=default_speed_fast)
	c.wait(20)

#Perform the acutal diazirine-NHS coupling reaction
for i in range(1):
	#Fill Reactor_1 bottom 
	c.move("DCM", "Reactor_1", filter1_dead_volume, speed=default_speed_fast, dest_port="bottom")
	total_volume_DMF += filter1_dead_volume 

	# Stir
	c.stirrer.set_stir_rate("Reactor_1", stirring_rate_default)
	c.stirrer.stir("Reactor_1")

	# Fill Reactor_1 with base in DCM
	total_volume_DIPEA_in_DCM += prime_tubing_to_filter1("DIPEA_in_DCM")
	c.move("DIPEA_in_DCM", "Reactor_1", volume_DIPEA_in_DCM, speed=default_speed_slow, dest_port="top")
	total_volume_DIPEA_in_DCM += volume_coupling_base 

	#Add NHS_diazirine
	c.move("NHS_diazirine", "Reactor_1", volume_NHS_diazirine, speed=default_speed_slow, dest_port="top")
	total_volume_NHS_diazirine += volume_NHS_diazirine
	clean_org(waste = "Waste_1")
	air_push_to_filter1()

	# Clean tubes with DCM
	clean_org_DCM(no_of_times=3, waste = "Waste_1")

	#Wait for reaction to complete
	c.camera.change_recording_speed(100)
	c.wait(wait_time_coupling)
	c.camera.change_recording_speed(20)

	#Drain filter
	drain_filter_1(filter1_dead_volume + volume_amino_acid + volume_coupling_base)
	c.stirrer.stop_stir("Reactor_1")
	dry("Reactor_1", 300)

########################################################################################################################
#                                                                                                                      #
#       PRE-CLEAVAGE WASH WITH DCM                                                                                     #
#                                                                                                                      #
#                                                                                                                      #
########################################################################################################################
c.logger.info("Pre-cleavage wash with DCM.")

total_volume_precleavage_solvent += filter_react("DCM", volume_precleavage_solvent, wait_time_wash_resin, no_of_times = no_of_resin_washes, fill_bottom = False, clean_backbone = "no")

c.logger.info("Drying resin under vacuum.")
dry("Reactor_1", wait_time_dry_resin)
